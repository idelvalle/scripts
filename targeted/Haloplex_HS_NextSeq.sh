...
MERGE='/home/nacho/Desktop/V3_Haloplex_HS_Jen/V3_FASTQ_files_for_Nacho/test/fastq_lane_merging.sh'

for f in */; do cd "$f" && bash ${MERGE} ; cd .. ; done
...
do
	...
	java -jar $SURECALLTRIMMER -fq1 ${R1} -fq2 ${R3} -halo -hs
...
for file in $(ls *.fastq.gz | rev | cut -c 32- | rev | uniq)
do
	R1=${file}*_R1*.gz
	R3=${file}*_R3*.gz

	bwa mem -t 16 -M $IDX ${R1} ${R3} > ${file}.sam
...
AMPLICON='/home/nacho/Desktop/V3_Haloplex_HS_Jen/04818-1520522278_Amplicons.bed'
...
java -jar $LOCATIT  -U -IS -OB -r -c 2500 -l ${AMPLICON} -o ${file%.*}.lc.bam ${file} ${file%.*}*R2*.fastq.gz
...
-R ${IDX} \
-I ${i}.recal.bam \
-L '/home/nacho/Desktop/V3_Haloplex_HS_Jen/04818-1520522278_Amplicons.bed' \
